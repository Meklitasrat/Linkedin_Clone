...
const EducationSection = ({ userData, isOwnProfile, onSave }) => {
	...
	const handleDeleteEducation = (school) => {
		setEducations(educations.filter((edu) => edu.school !== school));
	};
		...
		<div className='bg-white shadow rounded-lg p-6 mb-6'>
			...
			{educations.map((edu) => (
				<div key={edu._id} className='mb-4 flex justify-between items-start'>
					...
					{isEditing && (
						<button onClick={() => handleDeleteEducation(edu.school)} className='text-red-500'>
							<X size={20} />
						</button>
					)}
				</div>
